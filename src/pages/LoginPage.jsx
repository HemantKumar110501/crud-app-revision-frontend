...
const LoginPage = () => {
  ...
  const [formData,setFormData] = useState({
   ...
   const handleSubmit = (e) => {
    ...
    //checking for user to not enter empty space or leave the input field blank , is he does then show toast
    if(newUser.email.trim() === "" && newUser.username.trim() === "" && newUser.password.trim() === ""){
      ...
      //agar local storage mai users key mai data h kuch to wo return kro ya phir empty array return kro
      let allSignUpUsers = JSON.parse(localStorage.getItem("users")) || []

      //input field mai dale huwe email aur password ko localstorage se aaye email aur password k match krke validate kro.
      let authUser = allSignUpUsers.find((ele) => {
        return ele.email === newUser.email && ele.password === newUser.password
      })
      console.log(authUser);

      //agar validate ho jata h to if chalao warna else chalao 
      if(authUser){
        // toast message
